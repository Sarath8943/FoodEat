...
function Coupons() {
  ...
  const columns = [
    ...
    },
    {
      name: "Active",
      selector: (row) => (
        <span className={row.isActive ? "text-emerald-600 font-semibold" : "text-gray-500"}>
          {row.isActive ? "Yes" : "No"}
        </span>
      ),
      sortable: true,
      ...
      cell: (row) => (
        <div className="flex space-x-3">
          <button
            ...
            className="hover:scale-110 transition-transform duration-200"
          >
            <Pencil className="w-5 h-5 text-amber-500 hover:text-amber-700" />
          </button>
          <button
            onClick={() => handleDeleteCoupon(row._id)}
            title="Delete"
            className="hover:scale-110 transition-transform duration-200"
          >
            <Trash2 className="w-5 h-5 text-rose-500 hover:text-rose-700" />
          </button>
        </div>
      ),
    },
  ...
  return (
    <div className="min-h-screen px-4 py-10 bg-slate-100 dark:bg-slate-900 transition-colors duration-300">
      <div className="max-w-7xl mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-lg p-6 sm:p-10">
        <div className="flex justify-between items-center mb-8">
          <h2 className="text-3xl font-bold text-slate-800 dark:text-white">Manage Coupons</h2>
          <button
            onClick={handleAddCoupon}
            className="bg-teal-600 hover:bg-teal-700 text-white px-6 py-2 rounded-lg font-medium shadow-md transition duration-200 transform hover:scale-105"
          >
            + Add Coupon
          </button>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10">
            <p className="text-rose-500 text-lg font-medium">{error.message}</p>
            <button
              onClick={fetchData}
